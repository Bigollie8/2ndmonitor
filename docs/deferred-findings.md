...
**RESOLVED 2026-07-31**, with one deliberate change from the sketch above: live went **hover-only** (user's pick — the ambient 6-sandbox treatment was the marketplace-open near-crash), so `previewSourceFor` now returns only image/glyph/placeholder and a separate `canLivePreview()` gates a hover-mounted `LivePreview` in `CatalogCard`. All 27 visualizer bundles got generated previews (`scripts/preview-capture.ts` — runs each bundle's real main.js against the real sandbox shims same-document; canvas surfaces via `toDataURL` composited onto black, dom surfaces via SVG `foreignObject`) and were republished (the 12 already-published ones patch-bumped; superseded no-preview rows left in place — index is `ORDER BY created_at` and mergeCatalog is last-write-wins per id, so the new row wins). The 7 tile bundles the outage had stranded unpublished went live in the same batch. Remaining gap: those 7 tiles have no preview images (the declarative tile surface needs a different harness) — they fall back to letter blocks.
