...
### Task 3: Retire the five built-ins

**Files (FIVE tiles — see the rulings below):** modify `app/src/state/tileMeta.ts`, `app/src/state/layout.ts`, `app/src/App.tsx` (the `renderTile` dispatch); delete the five `*Tile.tsx` components.
...
**UPDATED AFTER TASK 2: this task retires FIVE tiles.** `homeAssistant` and `energy` were never ported - their host is user-supplied and `net:<host>` is a literal fixed at authoring time, so they are genuinely unexpressible under the current permission grammar. Retire only: `randomWiki`, `launches`, `githubPrs`, `phoneNotifs`, `birds`.

- [ ] **Step 2: Delete the built-ins**

Remove the five from `TILE_META`, `ALL_TILE_TYPES`, both default-rect maps and the `renderTile` dispatch, and delete their components. `TILE_META` is `Record<BuiltinTileType, TileMeta>`, so the compiler refuses an incomplete table — let `tsc -b` find every site.

- [ ] **Step 3: Seed**

Run: `cd app && npm run bundles:seed`. Confirm seven new tile seeds (seven bundles exist; only five built-ins retire), each exactly `manifest.json` + `view.json`.
...
- [ ] **Step 2:** Cold-start seeding — move the installed `tiles/` folder aside, launch, confirm all seven install on boot and appear in the content library.
- [ ] **Step 3:** Place each of the seven on the dashboard (both onThisDay variants coexist — built-in and bundle) and confirm it renders real data. **Report any that show an error or an empty state, by name.** For credentialed tiles without a credential, the correct result is the needs-setup state, not a blank or an error — confirm which you see.
