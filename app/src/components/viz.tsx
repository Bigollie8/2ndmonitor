...
import { animateFrame, createAnimatePace } from '../state/animatePace';
...
export function useAnimateGate(
  ...
  // Frame pumps retain their original closure; read changing inputs via refs.
  const inputs = useRef({ paused, liveRef });
  inputs.current = { paused, liveRef };
  const pace = useRef(createAnimatePace(performance.now()));
  return {
    shouldDraw(): boolean {
      const current = inputs.current;
      if (!animateFrame(performance.now(), pace.current,
        !!current.paused || isAppHidden(), current.liveRef?.current.live, vizMaxFps)) return false;
