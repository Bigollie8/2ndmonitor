...
pub struct VizFolder {
    ...
    /// `marketplace_install`) exists in the folder, else "local". A
    /// hand-authored draft — e.g. the template "+ New visualizer" writes, or
    /// any other folder a user drops in by hand — is always "local". The
    /// content registry (contentRegistry.ts) admits only "marketplace"
    /// entries into the public style catalog; "local" folders stay reachable
    /// through the Scripted style's own picker, the surface built for
    /// authoring them.
    pub source: String,
}
...
/// half-written draft or a missing payload file is diagnosable from the badge
/// alone. Mirrors the shape of `validateManifest` in src/sandbox/manifest.ts
/// (the TypeScript reference), restricted to the fields the Rust side can
/// check cheaply: id/name/version/api and the presence of `main.js`.
fn validate_folder(v: &serde_json::Value, folder_id: &str, dir: &std::path::Path) -> Option<String> {
    ...
    if !dir.join("main.js").is_file() {
        return Some("main.js is missing".to_string());
    }
    None
}
...
mod tests {
    ...
    fn folder_source_is_marketplace_with_a_valid_marker() {
        ...
        std::fs::write(
            ...
            json!({"id": "x", "version": "1.0.0", "kind": "visualizer", "installed_at": 1}).to_string(),
        )
        .unwrap();
    ...
    fn validate_folder_rejects_id_folder_mismatch() {
        ...
        std::fs::write(d.0.join("main.js"), "").unwrap();
    ...
    fn validate_folder_rejects_missing_payload_file() {
        let d = ScratchDir::new("no-mainjs");
        let v = json!({"id": "mine", "name": "N", "version": "1.0.0", "api": 1});
        let err = validate_folder(&v, "mine", &d.0).unwrap();
        assert_eq!(err, "main.js is missing");
    }
    ...
    fn folder_entry_reports_marketplace_source_and_no_error_for_a_valid_installed_folder() {
        ...
        std::fs::write(
            d.0.join("installed.json"),
            json!({"id": "mine", "version": "1.0.0", "kind": "visualizer", "installed_at": 1}).to_string(),
        )
        .unwrap();
    ...
    #[test]
    fn folder_entry_reports_local_source_for_a_hand_authored_draft() {
        let d = ScratchDir::new("draft");
        std::fs::write(d.0.join("main.js"), "").unwrap();
        std::fs::write(
            d.0.join("manifest.json"),
            json!({"id": "draft", "name": "My Visualizer", "version": "0.1.0", "api": 1}).to_string(),
        )
        .unwrap();
        let entry = folder_entry(&d.0, "draft".to_string());
